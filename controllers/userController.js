	...
	try {
		...
		await Model.findOne({ _id: id })
			...
			.catch((err) =>
				res
					.status(404)
					.json({ success: false, message: 'user not found', err }),
			);
	} catch (err) {
		return next(err);
	}
...
export async function editAdminRole(req, res) {
	...
	try {
		const user = Model.findOne({ _id });
		if (user) {
			if (role === 'admin') {
				Model.updateOne({ _id }, { role: 'superAdmin' }).then(
					(response) => {
				...
				Model.updateOne({ _id }, { role: 'admin' }).then((response) => {
					console.log(response);
					return res.json({ success: true });
				});
			} else {
